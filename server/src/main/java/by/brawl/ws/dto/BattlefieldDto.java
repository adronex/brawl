...
public class BattlefieldDto extends AbstractDto implements JsonDto {
    ...
    public BattlefieldDto(BattlefieldHolder battlefieldHolder, String receiverName) {

        myHeroes = Mappers.asList(battlefieldHolder.getBattleHeroes(receiverName, false),
                                  HeroDto::new);
        enemyHeroes = Mappers.asList(battlefieldHolder.getBattleHeroes(receiverName, true),
                                     HeroDto::new);

        battlefieldHolder.getBattleHeroes(receiverName, false)
                         .forEach(heroHolder -> {
                             heroSpells.put(heroHolder.getId(),
                                            heroHolder.getAllSpells()
                                                      .stream()
                                                      .map(SpellDto::new)
        ...
        battlefieldHolder.getBattleHeroes(receiverName, true)
                         .forEach(heroHolder -> {
                             heroSpells.put(heroHolder.getId(),
                                            heroHolder.getAllSpells()
                                                      .stream()
                                                      .filter(spellHolder -> battlefieldHolder
                                                              .getBattleLog()
                                                              .stream()
                                                              .filter(stepLog -> Objects
                                                                      .equals(stepLog.getSpellId(), spellHolder
                                                                              .getId()) && Objects
                                                                      .equals(stepLog.getCasterId(), heroHolder
                                                                              .getId()))
                                                              .count() > 0)
                                                      .map(SpellDto::new)
                                                      .collect(Collectors.toSet()));
        ...
        battlefieldHolder.getQueue().forEach(s -> {
            ...
            Boolean exposed = !enemy ||
                    battlefieldHolder.getBattleLog()
                                     .stream()
        ...
        battleLog = Mappers.asList(battlefieldHolder.getBattleLog(),
                                   (stepLog -> new StepLogDto(stepLog, receiverName)));
